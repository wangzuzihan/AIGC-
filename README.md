...
`(1)就在最近，我们获得了关于GPT4的最新消息。`GPT4模型是一个基于MoE架构的超大模型，参数量是GPT3的30倍！目前可以预知的是其能超过图灵测试，在推理过程中消耗的资源和算力并不会比GPT3大多少，甚至很有可能是一样的。Open AI 一直看好通用语言模型的发展，从其过去发布的 GPT 系列模型就可看出这一趋势。最近 Open AI 的 CEO Sam Altman 也在推特上说：”Language interfaces are going to be a big deal, I thik."
